import random
import os
import torch.backends.cudnn as cudnn
import torch.optim as optim
import torch.utils.data
from torch.autograd import Variable
from dataset import *
from torchvision import datasets
from torchvision import transforms
from lenet import *
from discriminator import *
from function import *
import numpy as np
from pretrain_src import *
from train_target import *

source_dataset_name = 'usps'
target_dataset_name = 'mnistm'
#checkpoint = "./usps_mnistm_1/"
#source_dataset_name = 'usps'
#target_dataset_name = 'mnistm'
print(source_dataset_name, "=>", target_dataset_name)
path = "../../hw3_data/digits"
load_src = True
load_target = False
checkpoint = "./usps_mnistm_2"

source_path = os.path.join(path, source_dataset_name)
target_path = os.path.join(path, target_dataset_name)

cuda = True
cudnn.benchmark = True
lr = 1e-4
batch_size = 128
image_size = 28
n_epoch_pre = 100
n_epoch = 2000

manual_seed = random.randint(1, 10000)
random.seed(manual_seed)
torch.manual_seed(manual_seed)
torch.cuda.manual_seed(manual_seed)
checkpoint = "./usps_mnistm_2"
"""
if target_dataset_name == 'usps':
    checkpoint = "./svhn_usps"
elif target_dataset_name == 'mnistm':
    checkpoint = "./usps_mnistm_1"
elif target_dataset_name == 'svhn':
    checkpoint = "./mnistm_svhn_1"
"""
if target_dataset_name == 'usps':
    
    transoform = [transforms.ToTensor()]
    ### source data ###
    dataset_source_train = Imgdataset(
        os.path.join(source_path, "train"),
        transform=transoform,
        train=True,
        channel=1
    )

    dataset_source_test = Imgdataset(
        os.path.join(source_path, "test"),
        transform=transoform,
        train=False,
        channel=1
    )

    ### target data ###

    dataset_target_train = Imgdataset(
        os.path.join(target_path, "train"),
        transform=transoform,
        train=True,
        channel=1
    )

    dataset_target_test = Imgdataset(
        os.path.join(target_path, "test"),
        transform=transoform,
        train=False,
        channel=1
    )
else:
    
    transoform = [transforms.ToTensor(), transforms.Normalize(
                                            mean=(0.5,0.5,0.5),
                                            std=(0.5,0.5,0.5))]
    ### source data ###
    dataset_source_train = Imgdataset(
        os.path.join(source_path, "train"),
        transform=transoform,
        train=True
    )

    dataset_source_test = Imgdataset(
        os.path.join(source_path, "test"),
        transform=transoform,
        train=False
    )

    ### target data ###

    dataset_target_train = Imgdataset(
        os.path.join(target_path, "train"),
        transform=transoform,
        train=True
    )

    dataset_target_test = Imgdataset(
        os.path.join(target_path, "test"),
        transform=transoform,
        train=False
    )
#print(len(dataset_source_train))
#print(len(dataset_target_train))

#exit()
### Init Modle ###
if target_dataset_name == "usps":
    src_encoder = init_model(LeNetEncoder_special())
    src_classifier = init_model(LeNetClassifier_special())
    target_encoder = init_model(LeNetEncoder_special())
    critic = init_model(Discriminator())

else:
    src_encoder = init_model(LeNetEncoder())
    src_classifier = init_model(LeNetClassifier())
    target_encoder = init_model(LeNetEncoder())
    critic = init_model(Discriminator())

if cuda:
    src_encoder = src_encoder.cuda()
    src_classifier = src_classifier.cuda()
    target_encoder = target_encoder.cuda()
    critic = critic.cuda()


if load_src: 
    print("=== Load pretrained Src model ===")
    src_encoder.load_state_dict(torch.load(os.path.join(checkpoint, "src_encoder.ckp")))
    src_classifier.load_state_dict(torch.load(os.path.join(checkpoint, "src_classifier.ckp")))
else:
    print("=== Train Src encoder and Classifier ===")
    src_encoder, src_classifier = train_src(src_encoder, src_classifier, dataset_source_train, dataset_source_test, n_epoch_pre, cuda, checkpoint)

### Eval source model ###
print("=== Evaluating classifier for source domain ===")
src_encoder.load_state_dict(torch.load(os.path.join(checkpoint, "src_encoder.ckp")))
src_classifier.load_state_dict(torch.load(os.path.join(checkpoint, "src_classifier.ckp")))
eval_src(src_encoder, src_classifier, dataset_source_test, cuda)


if load_target: 
    print("=== Load pretrained target model ===")
    target_encoder.load_state_dict(torch.load(os.path.join(checkpoint, "target_encoder.ckp")))
else:
    print("=== Train target encoder ===")
    target_encoder.load_state_dict(src_encoder.state_dict())
    target_encoder = train_target(src_encoder, src_classifier, target_encoder, critic, 
                                    dataset_source_train, dataset_target_train, dataset_target_test, n_epoch, cuda, checkpoint)

### Eval target model ###
print("=== Evaluating classifier for target domain ===")
target_encoder.load_state_dict(torch.load(os.path.join(checkpoint, "target_encoder.ckp")))
src_classifier.load_state_dict(torch.load(os.path.join(checkpoint, "src_classifier.ckp")))
eval_target(target_encoder, src_classifier, dataset_target_test, cuda)





exit()  
